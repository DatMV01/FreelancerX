...
function parseFromUrl<Entity>(
  ...
  if (sortsStr) {
    sortsStr.split(",").forEach((entry) => {
      const [key, dir] = entry.split(":");
      if (key && (dir === "ASC" || dir === "DESC")) {
        sorts[key as keyof Entity] = dir;
        hasSorts = true;
      }
    });
  }
  ...
  if (filtersStr) {
    filtersStr.split(",").forEach((entry) => {
      const [key, val] = entry.split(":");
      if (key && val !== undefined) {
        filters[key as keyof Entity] = parseValue(val);
        hasFilters = true;
      }
    });
  }
  ...
  if (fieldsStr) {
    fields.push(...(fieldsStr.split(",") as (keyof Entity)[]));
    hasFields = true;
  }
