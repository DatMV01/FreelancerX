// components/Checkout/Step1Review.tsx
...
export default function Step1Review({ onNext }: { onNext: () => void }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axiosInstanceV1.get(`/gig/${gigId}`);
        const { data } = response;
        setGig(data);
      } catch (error: any) {
        setError("Error fetching gig data: " + error.message);
      } finally {
        setLoading(false);
      }
    };

    if (gigId && packageTitle) fetchData();
  }, [gigId, packageTitle]);

  const getPackageInfo = (title: "basic" | "standard" | "premium") => {
    if (!gig?.pricingPackage || !Array.isArray(gig.pricingPackage)) return null;
    ...
    for (const pkg of gig.pricingPackage) {
      if ((pkg as any).type === "packageName") {
        ...
        if (value && value !== "") {
          const includedText = value === "x" ? pkg.package : `${pkg.package}: ${value}`;
          info.packageIncluded.push(includedText);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (validTitles.includes(packageTitle)) {
      const info = getPackageInfo(packageTitle as "basic" | "standard" | "premium");
      if (info) setPackageInfo(info);
    }
  }, [gig, packageTitle]);
