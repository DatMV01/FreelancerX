...
import { deleteFileById, uploadFile } from "@/features/files/file.api";
...
import { createGig, getGigById, updateGig } from "@/features/gig/gig.api";
...
const gigSchema = z.object({
  ...
  freelancer: z.any().optional().nullable(),
  tags: z
    .array(
      z.object({
        id: z.string().uuid(),
...
function FreelancerEditGigPage() {
  ...
  const { setValue, watch, handleSubmit, reset, trigger, formState } = form;
    ...
    async function fetchFreelancerData() {
      ...
      try {
        ...
        reset({
          ...
          tags: Array.from(data.tags).map((_: any) => ({
            ..._,
            createdAt: new Date(_.createdAt),
          })),
          medias: fillMissingFileSlots(data.medias),
        } as any);
      } catch (error) {
  ...
  const onSubmit = async (values: z.infer<typeof gigSchema>) => {
    ...
    for (const [slot, file] of Object.entries(
      ...
      const success = await deleteFileById(file.id);

      console.log(slot);
      console.log(file);
      console.log("Deleted", success);
      values.medias[slot as keyof typeof values.medias] = null;
    }

    for (const [slot, { from, to }] of Object.entries(
      fileChangesDetectedRef.current.updated,
    )) {
      const isDeleteSuccess = await deleteFileById(from.id);
    ...
    try {
      const response = await updateGig(values.id as any, values as any);
      const { slug } = response.data;

      toast.success(`Update gig successfully!`);
      setMessage({
        type: "success",
        message: `Update gig successfully!`,
      });
      ...
      setTimeout(() => {
        router.replace(
          `/dashboard/freelancer/gigs?page=1&pageSize=10&status=${values.status.toUpperCase()}`,
        );
      }, 7000);
    } catch (error: any) {
      if (
